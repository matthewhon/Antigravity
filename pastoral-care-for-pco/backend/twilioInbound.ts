...
export const handleInboundSms = async (req: any, res: any) => {
    ...
    try {
        ...
        // Use twilioSid as the document ID so duplicate webhook deliveries
        // (Twilio retries when it doesn't receive a timely 200) are idempotent:
        ...
        const messageId = smsSid
            ? `msg_sid_${smsSid.replace(/-/g, '')}`
        ...
        const costUsd = isMms ? 0.02 : segments * 0.0079;
        const usageId = smsSid
            ? `usage_inbound_sid_${smsSid.replace(/-/g, '')}`
        ...
        if (kw) {
            ...
            const resolvedPersonId = personMatch?.personId || convSnap.data()?.personId;
            if (kw.addToListId && resolvedPersonId) {
                log.info(`[Inbound SMS] Keyword "${kw.keyword}" matched — would add ${resolvedPersonId} to list ${kw.addToListId}`, 'system', { churchId, keyword: kw.keyword }, churchId);
