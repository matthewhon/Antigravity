...
export const CareContactPage: React.FC<CareContactPageProps> = ({ church, user, people, groups = [] }) => {
    ...
    const buildEligiblePeople = useCallback((
        ...
        // Tier 0: never attempted in any session (no touchpoint, never contacted) — sort most at-risk (lowest score) first
        // Tier 1: attempted but never successfully contacted (has touchpoint) — oldest attempt/touchpoint first
        ...
        const tierOf = (p: typeof eligible[0]) => {
            ...
            if (lastAttemptedAt.has(p.id)) return 1;  // tried but no answer (touchpoint only)
        ...
        return eligible
            .sort((a, b) => {
                ...
                if (ta === 1) {
                    // Both attempted-only — oldest attempt/touchpoint first
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <a
                        href="/care/reports?tab=sessions"
                        className="flex items-center gap-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-950/40 dark:hover:bg-indigo-900/60 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition-all"
                    >
