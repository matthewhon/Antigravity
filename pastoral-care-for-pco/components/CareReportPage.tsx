...
import { Download, Search, ChevronUp, ChevronDown, ChevronsUpDown, MessageSquare, Filter, X } from 'lucide-react';
...
export const CareReportPage: React.FC<CareReportPageProps> = ({
    ...
    // --- Build last-note map: personId → most recent PastoralNote ---
    const lastNoteMap = useMemo(() => {
        const m = new Map<string, PastoralNote>();
        // notes are fetched ordered by date desc, so first occurrence = most recent
        notes.forEach(n => {
            if (!m.has(n.personId)) m.set(n.personId, n);
        });
        return m;
    }, [notes]);

    // --- Note count map: personId → count ---
    ...
    const allRows = useMemo<EnrichedRow[]>(() => {
        ...
        return peopleData.allPeople
            ...
            .map(person => {
                ...
                // Higher urgency = needs contact more. Never-contacted + low score = highest.
                ...
                return { person, lastNote, noteCount, daysSinceContact: days, urgencyScore: urgency, followedUpRecently };
            });
    }, [peopleData, lastNoteMap, noteCountMap, followUpMap]);

    // --- Filter rows ---
    const filteredRows = useMemo(() => {
        return allRows.filter(({ person, lastNote, noteCount, daysSinceContact }) => {
            if (search) {
            ...
            if (membershipFilter !== 'all') {
                const m = person.membership || 'None';
                if (m !== membershipFilter) return false;
            }
            ...
            if (upcomingOnly) {
                const hasBd = isUpcomingAnnual(person.birthdate);
                const hasAnn = isUpcomingAnnual(person.anniversary);
                if (!hasBd && !hasAnn) return false;
            }
            return true;
        });
    }, [allRows, search, riskFilter, membershipFilter, lastContactFilter, hasNotesFilter, upcomingOnly]);
    ...
    const sortedRows = useMemo(() => {
        ...
        return [...filteredRows].sort((a, b) => {
            switch (sortKey) {
                ...
                case 'urgency':
                    return mult * (a.urgencyScore - b.urgencyScore);
                case 'riskScore':
                    return mult * ((a.person.riskProfile?.score ?? 0) - (b.person.riskProfile?.score ?? 0));
                case 'riskCategory': {
                    const order = { Healthy: 0, 'At Risk': 1, Disconnected: 2 };
                    const ca = order[a.person.riskProfile?.category as keyof typeof order] ?? 1;
                    const cb = order[b.person.riskProfile?.category as keyof typeof order] ?? 1;
                    return mult * (ca - cb);
                }
                case 'lastContact':
                case 'daysSince':
                    return mult * (a.daysSinceContact - b.daysSinceContact);
                case 'notesCount':
                    return mult * (a.noteCount - b.noteCount);
                case 'membership':
                    return mult * (a.person.membership || '').localeCompare(b.person.membership || '');
                default:
                    return 0;
            }
        });
    ...
    const stats = useMemo(() => {
        ...
        const avg = withDays.length > 0
            ...
            : null;
        return { never, over90, avg, total: filteredRows.length };
    }, [filteredRows]);
    ...
    const hasActiveFilter = search || riskFilter !== 'all' || membershipFilter !== 'all'
        || lastContactFilter !== 'all' || hasNotesFilter !== 'all' || upcomingOnly;
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-0.5">
                        Who needs to be contacted — and when was the last time we reached out?
                    </p>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    ...
                    { label: 'Avg Days Since Contact', value: stats.avg !== null ? `${stats.avg}d` : '—', color: 'text-indigo-500', bg: 'bg-indigo-50 dark:bg-indigo-950/20 border-indigo-100 dark:border-indigo-900/30' },
                    { label: 'People in View', value: stats.total, color: 'text-slate-700 dark:text-slate-200', bg: 'bg-slate-50 dark:bg-slate-800/60 border-slate-100 dark:border-slate-700' },
                ].map(card => (
            ...
            <div className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 bg-white dark:bg-slate-900 z-10 shadow-sm">
                            <tr>
                                ...
                                <ThCol label="Type" col="lastContact" />
                                <ThCol label="Days Since" col="daysSince" />
                                <ThCol label="Notes" col="notesCount" />
                                <ThCol label="Urgency" col="urgency" />
                                <th className="p-2 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800 whitespace-nowrap">
                                    Birthday
                                </th>
                                <th className="p-2 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800 whitespace-nowrap">
                                    Anniversary
                                </th>
                                <th className="p-2 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 dark:border-slate-800 text-right pr-4 whitespace-nowrap">
                                    Actions
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            {sortedRows.length === 0 ? (
                                <tr>
                                    <td colSpan={12} className="py-16 text-center text-slate-400 text-sm font-bold">
                                        No people match your current filters.
                                    </td>
                                </tr>
                            ) : sortedRows.map(({ person, lastNote, noteCount, daysSinceContact, urgencyScore, followedUpRecently }) => {
                                ...
                                return (
                                    <tr
                                        key={person.id}
                                        className={`group border-b border-slate-50 dark:border-slate-800/50 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/40 ${
                                            isNever ? 'border-l-2 border-l-rose-300 dark:border-l-rose-800/60' : ''
                                        }`}
                                    >
                                        {/* Person */}
                                            ...
                                            <div className="flex items-center gap-3">
                                                {person.avatar ? (
                                                    <img
                                                        ...
                                                        width="32" height="32"
                                                        loading="lazy"
                                                        className="w-8 h-8 rounded-full object-cover bg-slate-200 flex-shrink-0"
                                                    />
                                                ) : (
                                                    <div className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-xs uppercase flex-shrink-0">
                                                        {person.name.substring(0, 2)}
                                                    </div>
                                                )}
                                                <div className="min-w-0">
                                                    <div className="flex items-center gap-1.5 flex-wrap">
                                                        <p className="text-sm font-bold text-slate-900 dark:text-white truncate max-w-[140px]" title={person.name}>
                                                            {person.name}
                                                        </p>
                                                        {followedUpRecently && (
                                                            <span className="text-[9px] font-black uppercase tracking-wide bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 px-1.5 py-0.5 rounded-full whitespace-nowrap">
                                                                ✓ Done
                                                            </span>
                                                        )}
                                                        {isOverdue && !followedUpRecently && (
                                                            <span className="text-[9px] font-black uppercase tracking-wide bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 px-1.5 py-0.5 rounded-full whitespace-nowrap">
                                                                ⚠️ Overdue
                                                            </span>
                                                        )}
                                                    </div>
                                        ...
                                        <td className="p-2 text-center">
                                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wide whitespace-nowrap ${riskBadge}`}>
                                                {category}
                                            </span>
                                        </td>
                                        ...
                                        {/* Membership */}
                                        <td className="p-2 text-center text-[11px] font-bold text-slate-500 dark:text-slate-400 whitespace-nowrap">
                                            {person.membership || '—'}
                                        </td>
                                        ...
                                        {/* Days since */}
                                        <td className={`p-2 text-center text-sm ${daysColor} whitespace-nowrap`}>
                                            {daysDisplay}
                                        </td>
                                        ...
                                        <td className="p-2 text-center">
                                            {noteCount > 0 ? (
                                                <span
                                                    title={lastNote?.content ? `Last note: "${lastNote.content.slice(0, 80)}…"` : undefined}
                                                    className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-indigo-100 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 text-[10px] font-black cursor-default"
                                                >
                                                    {noteCount}
                                                </span>
                                            ) : (
                                        ...
                                        <td className="p-2 text-center">
                                            <div className="w-16 h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mx-auto">
                                                <div
                                                    className={`h-full rounded-full ${urgencyColor} transition-all`}
                                        ...
                                        <td className="p-2 text-center">
                                            {bd ? (
                                                <span className={`text-[10px] font-bold whitespace-nowrap ${bdUpcoming ? 'text-pink-600 dark:text-pink-400' : 'text-slate-400'}`}>
                                                    {bdUpcoming && '🎂 '}{bd}
                                                </span>
                                        ...
                                        <td className="p-2 text-center">
                                            {ann ? (
                                                <span className={`text-[10px] font-bold whitespace-nowrap ${annUpcoming ? 'text-rose-600 dark:text-rose-400' : 'text-slate-400'}`}>
                                                    {annUpcoming && '💍 '}{ann}
                                                </span>
                                            ) : <span className="text-slate-300 dark:text-slate-600 text-[10px]">—</span>}
                                        </td>
                                        ...
                                        <td className="p-2 pr-4 text-right">
                                            <button
                                                ...
                                                className="opacity-0 group-hover:opacity-100 w-7 h-7 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 hover:bg-indigo-600 text-indigo-500 hover:text-white flex items-center justify-center ml-auto transition-all"
                                            >
                ...
                <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                        ...
                        {hasActiveFilter && <span className="text-indigo-500"> · Filtered</span>}
                    </p>
