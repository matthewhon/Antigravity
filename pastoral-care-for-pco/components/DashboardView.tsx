  ...
  const renderWidget = (id: string) => {
      ...
      switch(id) {
          ...
          case 'people_engagement':
              return peopleData ? (
                  <WidgetWrapper title="Engagement" onRemove={() => handleRemoveWidget(id)} source="People">
                      <div className="h-64">
                          <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} debounce={1}>
                              <PieChart>
                                  <Pie 
                                    data={peopleData.engagementData} 
                                    innerRadius={60} 
                                    outerRadius={80} 
                                    paddingAngle={5} 
                                    dataKey="value"
                                  >
                                      {peopleData.engagementData.map((entry, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
                                  </Pie>
          ...
          case 'gender':
              return peopleData ? (
                  <WidgetWrapper title="Gender" onRemove={() => handleRemoveWidget(id)} source="People">
                      <div className="h-64">
                          <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} debounce={1}>
                              <PieChart>
                                  <Pie 
          ...
          case 'age':
              return peopleData ? (
                  <WidgetWrapper title="Age" onRemove={() => handleRemoveWidget(id)} source="People">
                      <div className="h-40">
                          <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} debounce={1}>
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {safeVisibleWidgets.map(id => {
              ...
              if (['people_stats', 'keyMetrics', 'groups_stats', 'services_stats', 'ai_insights'].includes(id)) {
