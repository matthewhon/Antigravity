...
export const EmailBuilder: React.FC<EmailBuilderProps> = ({
  ...
  const [addingBlockAtIndex, setAddingBlockAtIndex] = useState<number | 'end' | 'empty' | null>(null);
  ...
  const insertBlockHelper = (prev: EmailBlock[], newBlock: EmailBlock, insertAfterIndex?: number) => {
            ...
            {([
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const newBlock: EmailBlock = {
                    id: `block_${Date.now()}_${Math.random().toString(36).slice(2,5)}`, type: 'columns',
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            <SortableContext items={blocks.map(b => b.id)} strategy={verticalListSortingStrategy}>
              {blocks.length === 0 ? (
                addingBlockAtIndex === 'empty' ? (
                  <div className="flex flex-col items-center justify-center h-64 border-2 border-slate-200 dark:border-slate-700 rounded-2xl p-6">
                    <p className="text-sm font-medium text-slate-500 mb-4">Choose a block to start</p>
                    ...
                    <button onClick={() => setAddingBlockAtIndex(null)} className="mt-4 text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">Cancel</button>
                  </div>
                ) : (
                  <div
                    ...
                    onClick={() => setAddingBlockAtIndex('empty')}
                  >
                    <Plus size={28} className="mb-2" />
                    <p className="text-sm font-medium">Click a block in the sidebar to get started</p>
                    <p className="text-xs mt-1">or click here to add a block</p>
                  </div>
                )
              ) : (
                  ...
                  {blocks.map((block, idx) => (
                    <React.Fragment key={block.id}>
                      <SortableCanvasBlock
                        ...
                        churchId={churchId}
                      />
                      {/* "+ Add block" between rows — visible on hover or when adding */}
                      <div className={`flex items-center gap-2 transition min-h-[16px] py-1 ${addingBlockAtIndex === idx ? 'opacity-100' : 'opacity-0 hover:opacity-100 group/add'}`}>
                        <div className="flex-1 h-px bg-slate-200 dark:bg-slate-700 group-hover/add:bg-indigo-300" />
                        {addingBlockAtIndex === idx ? (
                          <div className="flex flex-wrap gap-1 justify-center bg-white dark:bg-slate-900 p-1 rounded-lg border border-indigo-200 dark:border-indigo-700 shadow-sm z-10">
                            {BLOCK_DEFS.map(def => (
                              <button
                                key={def.type}
                            ...
                            <button onClick={() => setAddingBlockAtIndex(null)} className="px-2 py-1 rounded text-[10px] text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">Cancel</button>
                          </div>
                        ) : (
                          <button
                            onClick={() => setAddingBlockAtIndex(idx)}
          ...
          {blocks.length > 0 && (
            <div className="pl-8 mt-2 pb-8">
              {addingBlockAtIndex === 'end' ? (
                <div className="flex flex-wrap gap-2 justify-center p-3 bg-white dark:bg-slate-900 rounded-xl border border-indigo-200 dark:border-indigo-700 shadow-sm">
                  {BLOCK_DEFS.map(def => (
                    <button
                      key={def.type}
                  ...
                  <button onClick={() => setAddingBlockAtIndex(null)} className="flex items-center justify-center p-2 rounded-lg text-xs text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">Cancel</button>
                </div>
              ) : (
                <button
                  onClick={() => setAddingBlockAtIndex('end')}
