...
export const PastoralEngagementModal: React.FC<PastoralEngagementModalProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div 
        className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-4xl max-h-[92vh] flex flex-col border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Modal Top Header */}
        <div className="p-5 sm:p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-800/30 shrink-0">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                  Pastoral Engagement Playbook
                </span>
              </div>
              <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400 mt-0.5 flex-wrap">
                {email && <span className="flex items-center gap-1"><Mail className="w-3 h-3 text-slate-400" /> {email}</span>}
                {phone && <span className="flex items-center gap-1"><Phone className="w-3 h-3 text-slate-400" /> {phone}</span>}
        ...
        <div className="px-6 py-3 bg-slate-100/70 dark:bg-slate-800/60 border-b border-slate-200/60 dark:border-slate-800 flex items-center gap-3 overflow-x-auto shrink-0 text-xs">
          <span className="font-bold text-slate-400 text-[11px] uppercase tracking-wider shrink-0">
            Assessment Results:
          </span>

          {giftsResponse && (
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-xs shrink-0">
              <Sparkles className="w-3.5 h-3.5 text-indigo-600" />
              <span className="font-bold text-slate-700 dark:text-slate-200">
                Spiritual Gift: <strong>{giftsResponse.primaryGift}</strong>
              </span>
          ...
          {discResponse && (
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-xs shrink-0">
              ...
              <span className="font-bold text-slate-700 dark:text-slate-200">
                DISC: <strong>{discResponse.styleCode}</strong> ({discProfile?.name?.split('(')[0]?.trim() || 'Pattern'})
              </span>
            </div>
          )}
          ...
          {mbtiResponse && (
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-xs shrink-0">
              ...
              <span className="font-bold text-slate-700 dark:text-slate-200">
                MBTI: <strong>{mbtiResponse.mbtiType}</strong> ({mbtiProfile?.name?.split('/')[0]?.trim()})
              </span>
            </div>
          )}
        </div>
        ...
        {/* Navigation Tabs */}
        <div className="px-6 border-b border-slate-200 dark:border-slate-800 flex items-center gap-2 overflow-x-auto shrink-0 bg-white dark:bg-slate-900">
          <button
            type="button"
          ...
          >
            ...
            <span>Shepherding & Motivations</span>
          </button>
          ...
          >
            ...
            <span>Ministry Fit & Team Placement</span>
          </button>
          ...
          >
            ...
            <span>One-Click Outreach Scripts</span>
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-6">
          ...
          {activeTab === 'communication' && (
            <div className="space-y-6 animate-in fade-in duration-200">
              ...
              <div className="bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/80 rounded-2xl p-5 space-y-3">
                <div className="flex items-center gap-2">
              ...
              {/* What to Avoid */}
              <div className="bg-rose-50/70 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/80 rounded-2xl p-5 space-y-3">
                <div className="flex items-center gap-2">
                  <AlertCircle className="w-5 h-5 text-rose-600 dark:text-rose-400 shrink-0" />
                  <h4 className="text-sm font-black text-rose-950 dark:text-rose-200 uppercase tracking-wider">
          ...
          {activeTab === 'shepherding' && (
            <div className="space-y-6 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Core Motivations */}
                <div className="p-5 rounded-2xl bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/80 space-y-3">
                  <div className="flex items-center gap-2">
              ...
              <div className="p-5 rounded-2xl bg-indigo-50/70 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800/80 space-y-2">
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {(giftsProfile?.def?.recommendedServingAreas || discProfile?.idealServingRoles || mbtiProfile?.idealServingRoles || [
                    ...
                    <div key={idx} className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center gap-3">
            ...
            <div className="space-y-4 animate-in fade-in duration-200">
              ...
              <div className="space-y-4">
                {outreachScripts.map((script, idx) => (
                  <div key={idx} className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 space-y-3">
                    ...
                    <div className="p-3.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-mono select-all">
                      {script.text}
                    </div>
                  </div>
                ))}
        ...
        {/* Modal Footer */}
        <div className="p-4 sm:p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex items-center justify-between gap-4 shrink-0">
