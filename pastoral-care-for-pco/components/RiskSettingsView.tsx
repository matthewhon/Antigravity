...
const RiskSettingsView: React.FC<RiskSettingsViewProps> = ({ settings, onSave }) => {
  const [localSettings, setLocalSettings] = useState<RiskSettings>(settings || DEFAULT_RISK_SETTINGS);
  ...
  const handleWeightChange = (key: keyof RiskSettings['weights'], val: string) => {
  ...
  const handleThresholdChange = (key: keyof RiskSettings['thresholds'], val: string) => {
    const num = parseInt(val, 10);
    setLocalSettings(prev => ({
        ...prev,
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-200 dark:border-slate-800 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="bg-white dark:bg-slate-850 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
            <div className="flex justify-between items-center mb-6">
                <h4 className="font-black text-indigo-900 dark:text-indigo-300">Engagement Factors</h4>
            ...
            <div className="space-y-6">
                ...
                />
                <WeightSlider 
                    label="Serving / Volunteering" 
                    description="Impact of being scheduled in plans."
                ...
                <WeightSlider 
                    ...
                    value={localSettings.weights.giving} 
                    onChange={(v) => handleWeightChange('giving', v)} 
                />
                <WeightSlider 
                    label="Membership Status" 
                    description="Bonus for formal membership."
                    value={localSettings.weights.membership} 
                    onChange={(v) => handleWeightChange('membership', v)} 
                />
            </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-850 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-between transition-colors">
            <div>
                <h4 className="font-black text-indigo-900 dark:text-indigo-300 mb-6">Categorization Thresholds</h4>
                ...
                <div className="space-y-8 relative">
                    ...
                    <div className="pl-8 relative">
                        <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wide">Healthy Range</span>
                        ...
                        />
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-2">People scoring above {localSettings.thresholds.healthyMin} are considered Healthy.</p>
                    </div>

                    {/* At Risk */}
                    <div className="pl-8 relative">
                        <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-bold text-amber-500 dark:text-amber-400 uppercase tracking-wide">At Risk Range</span>
                        ...
                        />
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-2">People scoring between {localSettings.thresholds.atRiskMin} and {localSettings.thresholds.healthyMin} are flagged as At Risk.</p>
                    </div>

                    {/* Disconnected */}
                    <div className="pl-8 relative">
                        <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-bold text-rose-500 dark:text-rose-400 uppercase tracking-wide">Disconnected Range</span>
                        ...
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-2">Anyone scoring below {localSettings.thresholds.atRiskMin} is considered Disconnected.</p>
                    </div>
                </div>
            </div>
