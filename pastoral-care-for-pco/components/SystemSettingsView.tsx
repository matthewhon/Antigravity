    ...
    <div className="space-y-8 animate-in fade-in">
        ...
        {activeTab === 'Configuration' && (
            <div className="space-y-6">
                ...
                {/* ── Row 1: Planning Center | External APIs ──────────────────── */}
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                    ...
                    <div className={`px-8 py-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 ${
                        ...
                        <div className="flex items-center gap-4">
                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg shadow-sm ${
                                settings.emailProvider === 'postmark' ? 'bg-violet-600' : 'bg-emerald-600'
                            }`}>
                                ✉️
                            </div>
                            <div>
                                ...
                                <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                                    {settings.emailProvider === 'postmark'
                                        ? 'Postmark is the active provider — each tenant gets an isolated Server for reputation separation.'
                                        : 'SendGrid is the active provider — each tenant gets an isolated Subuser for reputation separation.'}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            {settings.emailProvider === 'postmark' ? (
                                settings.postmarkApiKey
                                    ? <span className="text-[10px] font-black bg-violet-600 text-white px-3 py-1 rounded-full">POSTMARK ✓ ACTIVE</span>
                                    : <span className="text-[10px] font-black bg-amber-500 text-white px-3 py-1 rounded-full">POSTMARK — NEEDS KEY</span>
                            ) : (
                                settings.sendGridApiKey?.startsWith('SG.')
                                    ? <span className="text-[10px] font-black bg-emerald-600 text-white px-3 py-1 rounded-full">SENDGRID ✓ ACTIVE</span>
                                    : <span className="text-[10px] font-black bg-amber-500 text-white px-3 py-1 rounded-full">SENDGRID — NEEDS KEY</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="p-8">
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-1">
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    >
                                        <div className="flex items-center gap-2 mb-1">
                                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                                                (!settings.emailProvider || settings.emailProvider === 'sendgrid') ? 'border-emerald-500 bg-emerald-500' : 'border-slate-300 dark:border-slate-600'
                                            }`}>
                                    ...
                                    >
                                        <div className="flex items-center gap-2 mb-1">
                                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                                                settings.emailProvider === 'postmark' ? 'border-violet-500 bg-violet-500' : 'border-slate-300 dark:border-slate-600'
                                            }`}>
                            ...
                            {/* Right: SendGrid Credentials */}
                            <div className={`lg:col-span-1 space-y-4 ${
                                settings.emailProvider === 'postmark' ? 'opacity-40 pointer-events-none' : ''
                            }`}>
                                <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                                <input
                                    type="text"
                                    value={settings.quickbooksRedirectUri ?? ''}
                                    onChange={e => handleChange('quickbooksRedirectUri', e.target.value)}
                                    placeholder={`${window.location.origin}/api/quickbooks/callback`}
                                    className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 font-mono text-xs text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-emerald-500 select-all"
                                />
                                <div className="flex gap-2 shrink-0">
                                    <button
                                        type="button"
                                        onClick={() => {
                                            const currentUri = `${window.location.origin}/api/quickbooks/callback`;
                                            handleChange('quickbooksRedirectUri', currentUri);
                                            setMessage({ type: 'success', text: `Redirect URI set to ${currentUri} — click "Save Settings" below to persist!` });
                                        }}
                                        className="px-3 py-2 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 rounded-xl text-xs font-bold transition-colors whitespace-nowrap"
                                        title="Auto-fill with current app origin"
                                    >
                                        Use Current Origin
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => {
                                            const copyVal = settings.quickbooksRedirectUri || `${window.location.origin}/api/quickbooks/callback`;
                                            navigator.clipboard.writeText(copyVal);
                                            setMessage({ type: 'success', text: 'QuickBooks Redirect URI copied to clipboard!' });
                                        }}
                                        className="px-3 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 rounded-xl text-xs font-bold transition-colors whitespace-nowrap"
                                    >
                                        Copy URI
                                    </button>
                                </div>
                            </div>
                            <p className="text-[10px] text-slate-400 mt-1">Leave blank to auto-detect your domain, or enter an explicit URL and click <strong>Save Settings</strong>. Add this exact URI to your Intuit App settings.</p>
                        </div>
                    </div>
