...
export const BroadcastPermissionsTab: React.FC<BroadcastPermissionsTabProps> = ({ churchId, church, allUsers, onUpdateChurch }) => {
                            ...
                            return (
                                <tr key={item.id} className={`${isSelected ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'} transition`}>
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        >
                                            {!hasAccess && <span title="Restricted by default" className="shrink-0"><ShieldAlert size={14} className="text-amber-500" /></span>}
                                            {item.type === 'list' ? (
