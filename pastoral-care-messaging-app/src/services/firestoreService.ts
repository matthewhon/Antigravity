...
class FirestoreService {
  ...
  async createTenantUser(churchId: string, userData: any): Promise<void> {
      ...
      const defaultBase = Capacitor.isNativePlatform() 
          ...
          : '';
      const apiBase = (settings.apiBaseUrl || defaultBase).replace(/\/$/, '');
      const res = await fetch(`${apiBase}/user/create`, {
          method:  'POST',
  ...
  async getServicesAnalytics(churchId: string, filter: ServicesFilter): Promise<ServicesDashboardData> {
      // Stubbed in mobile app to avoid date-fns/leaflet dependencies
      return {} as ServicesDashboardData;
  }
  ...
  async recalculateGlobalBenchmarks() {
      // Stubbed in mobile app to avoid date-fns/leaflet dependencies
      console.warn("recalculateGlobalBenchmarks is disabled in the mobile app");
  }
  ...
  async submitPollResponse(response: PollResponse): Promise<void> {
    try {
      ...
      batch.update(doc(db, 'polls', response.pollId), {
        totalResponses: ((await this.getPoll(response.pollId))?.totalResponses || 0) + 1,
        updatedAt: Date.now()
      });
      await batch.commit();
    } catch (e) {
  ...
  async saveTenantFile(file: any): Promise<void> {
    try {
      const sanitized = this.deepSanitize(file);
      await setDoc(doc(db, 'tenantFiles', file.id), sanitized, { merge: true });
    } catch (e) {
      this.handleFirestoreError(e);
      throw e;
    }
