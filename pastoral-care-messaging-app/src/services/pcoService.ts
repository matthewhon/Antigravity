...
const getProxyUrl = async (): Promise<string> => {
    ...
    try {
        ...
        cachedApiBaseUrl = (s.apiBaseUrl || 'https://pastoralcare.barnabassoftware.com').replace(/\/$/, '');
        return cachedApiBaseUrl + '/pco/proxy';
    } catch {
        return 'https://pastoralcare.barnabassoftware.com/pco/proxy';
    }
};
